# This Python file is used to organize the "Cosine Similarity" page, including its
...
st.text("We have created a separate DataFrame of “ranked pairs,” which consists of each pair of songs with a cosine similarity of 0.95 or higher. (Any song in this list of paired songs is selectable in the search bar below!) By merging this DataFrame with that of our original songs, we are able to provide the song and artist name of songs similar to the user’s song input, with the Spotify URL for the user to listen to a preview of the music. Give it a try for yourself!")
...
# Loading our "songs" df and "cosine_ranked_pairs" df, using st.cache_data to save the 
...
@st.cache_data
def load_cos_parquet(file_path: str):
    """Load parquet file and cache the result."""
    return pd.read_parquet(file_path)
ranked_pairs = load_cos_parquet("cos_ranked_pairs.parquet")
